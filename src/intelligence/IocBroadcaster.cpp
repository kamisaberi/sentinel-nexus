...
void IocBroadcaster::broadcast_threat(const ThreatIndicator& threat, uint64_t ttl_seconds) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    rule.set_target_subnet_mask(32); // Individual IPv4 host
    ...
    // Fan-out to all appliances (excluding the one that originally detected it)
